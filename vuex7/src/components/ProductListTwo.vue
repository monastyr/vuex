...
<div id="product-list-two">
    ...
    <ul>
        ...
        <button v-on:click="reducePriceMutations(4)">Reduce Price</button>
        <!--odejmuje od calej wartości 4; ale ponieważ widzimy tylko połowę ceny to z $10 redukuje się na $8-->
...
export default {
    ...
    methods:{
        ...
        reducePriceMutations: function(amount){ //mutacje ze globalnego cetral store
            // call commiting mutations - wywyołanie angażowania mutacji (po polsku źle to brzmi)

            // this.$store.commit('reducePrice'); Dobra praktywa - zamiast wywolywac bezposredno mutacji w komponencie lepiej wywołać actions,
            //która w sobie korzytsa z mutacji
            this.$store.dispatch('reducePrice', amount); //dispatching an actions
            //odejmuje od calej funkcji 4; ale ponieważ widzimy tylko połowę ceny to z $10 redukuje się na $8
            //pierwszy parametr to nazwa akcji, drugi argument funkcji
